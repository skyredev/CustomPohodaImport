...
class PohodaSyncReceivedInvoices implements JobDataLess
{
	...
	{
		$receivedInvoiceIds = $this->pohoda->getIdsToSync($this->pohoda->getReceivedInvoiceEntity());

		$this->debug('Received Invoices to sync count: ' . count($receivedInvoiceIds));

		foreach ($receivedInvoiceIds as $invoiceId) {
			try {
				$invoice = $this->pohoda->getEntityToSync($invoiceId?->id, $this->pohoda->getReceivedInvoiceEntity());
	...
	private function generateXmlForAccount(Entity $invoice): string
	{
		...
		$symconst = htmlspecialchars($invoice->get($this->pohoda->getReceivedInvoiceSymConst()));
		$accountNo = htmlspecialchars($invoice->get('bankaccount'));
		$bankCode = htmlspecialchars($invoice->get('bankcode'));
		$symvar = htmlspecialchars($invoice->get($this->pohoda->getReceivedInvoiceSymVar()));
		$symspec = htmlspecialchars($invoice->get('specificsymbol'));
		$date = htmlspecialchars($invoice->get($this->pohoda->getReceivedInvoiceDate()));
